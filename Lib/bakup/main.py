...
def ProsesStoreData(data):
    StoreData = pd.DataFrame(columns=['ID', 'Username', 'Date', 'Retweet', 'Tweet', 'Hashtags', 'RT', 'SA', 'Float'])
    ...
    for index, twit in enumerate(data):
        ...
        if 'retweeted_status' in twit:
            b = clean_tweet(twit['retweeted_status']['full_text'])
        else:
            b = ''
